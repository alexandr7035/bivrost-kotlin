package io.swisseth.solidity.plugin

import io.swisseth.solidity.AbiParser
...
open class SwissethExtension @Inject constructor(objects: ObjectFactory) {
...
@Suppress("unused")
fun Project.solidityTypes(configure: SwissethExtension.() -> Unit) {
    extensions.configure("solidityTypes", configure)
}

class SolidityTypesPlugin : Plugin<Project> {
    override fun apply(project: Project) {
        val ext = project.extensions.create<SwissethExtension>("solidityTypes")
        ...
        val generateTask = project.tasks.register<GenerateAbiWrapperTask>("generateAbiWrapper") {
            if (!ext.packageName.isPresent) {
                throw GradleException(
                    "SwissETH: packageName is not set in solidityTypes extension. " +
        ...
        if (kmpExt != null) {
            ...
            project.logger.lifecycle("Swisseth: generating ABI wrappers for KMP commonMain")
        } else {
            ...
            if (androidExt != null) {
                ...
                project.logger.lifecycle("Swisseth: generating ABI wrappers for $platformName")
            } else {
                project.logger.warn("Swisseth: no KMP or Android extension found, skipping sourceSet registration")
            }
        }
